...
data TypeCheckError
  ...
  | InvalidTypeForOpError { invalidType :: Maybe Type
                          , forOperator :: ExpressionCategory }
...
instance TypeCheckable TypeName where
  typeCheck symtbl (TypeName (Alias typename) typeval) =
    case typeCheck symtbl typeval of
      Right _ -> 
        case addEntry
            symtbl
            (IdOrType typename)
...
instance TypeCheckable Variable where
  ...
  typeCheck symtbl (Variable (i:is) maybe_type (e:es)) =
    case maybe_type of
      (Just _) ->
        case rightSideHelper symtbl e of
          Right (t, symtbl'') ->
                    case varDecHelper symtbl'' maybe_type of
                        Nothing ->
...
rightSideHelper :: SymbolTable -> Expression -> Either (TypeCheckError, SymbolTable) (Maybe Type, SymbolTable)
rightSideHelper s (Id i) = case lookupIdentifier s i of
                             Right e -> case typeCategory e of
                                         CategoryType -> Left (TypeCategoryError, s)
                                         _ -> typeCheck s i
rightSideHelper s e = typeCheck s e
...
instance TypeCheckable Type
  -- Or should this return typeChek symtbl (IdOrType s)?
                                                         where
  typeCheck symtbl (Alias s) = typeCheck symtbl (IdOrType s)
  typeCheck symtbl (Array t i) = typeCheck symtbl t
  typeCheck symtbl (Slice t) = typeCheck symtbl t
  typeCheck symtbl (Struct i) = Right (Nothing, symtbl)


structTypeCheckHelper :: SymbolTable -> [([Identifier], Type)] -> Either (TypeCheckError, SymbolTable) (Maybe Type, SymbolTable)
structTypeCheckHelper s [] = Right (Nothing, s)
structTypeCheckHelper symtbl ((is,t):xs) =
  case typeCheckList symtbl is of
    Right (_, symtbl') ->
      case typeCheck symtbl' t of
        Right (_,symtbl'') -> structTypeCheckHelper symtbl'' xs
        Left err -> Left err
    Left err -> Left err
--
instance TypeCheckable SimpleStmt where
  ...
  typeCheck symtbl (Incr iden) =
    typeCheckElemOf
      symtbl
      iden
      [(Alias "rune"), (Alias "int"), (Alias "float64")]
  ...
  case popFrame symtbl of
    Right (f, ps) ->
      case hasKey f iden of
        False ->
          case iden of
            IdOrType "_" -> Left IllegalBlankIdentifier
            IdArray "_" _ -> Left IllegalBlankIdentifier
            _ ->
                case typeCheck symtbl expr of
                    Right (t, symtbl') ->
...
typeAssignable :: SymbolTable -> Maybe Type -> Maybe Type -> Bool
typeAssignable symtbl t1 t2 =
  case t2 of
    ...
    Just (Slice (Alias t)) ->
      case getBaseType symtbl (IdOrType t) of
        Right (t, _) -> assertTypeEqual t1 t
    Just (Array (Alias t) _) ->
      case getBaseType symtbl (IdOrType t) of
        Right (t, _) -> assertTypeEqual t1 t
    ...
    Right (Just x@(Alias x'), symtbl') ->
      if (t' `elem` ["int", "float64", "rune", "bool"] ||
          typeOrBaseElem
            symtbl
            t
            [(Alias "int"), (Alias "float64"), (Alias "rune"), (Alias "bool")]) &&
...
typeOrBaseElem :: SymbolTable -> Type -> [Type] -> Bool
...
typeOrBaseElem symtbl _ ts = False  
...
binaryAliasHelper :: SymbolTable -> (Type, Type) -> [(Type, Type)] -> Bool
...
binaryCheck tList ExpBoolean (Just (Alias str1)) symtbl =
  case getBaseType symtbl (IdOrType str1) of
    Right (Just t1', symtbl1) ->
      case ((t1', t1') `elem` tList) ||
           (binaryAliasHelper symtbl (t1', t1') tList) of
        True -> Right (Just (Alias "bool"), symtbl1)
        False -> Left (InvalidTypeForOpError (Just t1') ExpBoolean, symtbl1)
    ...
    (Alias s1) ->
      case getBaseType symtbl (IdOrType s1) of
        (Right (Just (Alias s1), symtbl1)) ->
          case ((Alias s1, Alias s1) `elem` tList) ||
               (binaryAliasHelper symtbl (Alias s1, Alias s1) tList) of
            True -> Right (Just (Alias "bool"), symtbl1)
            False -> Left (InvalidTypeForOpError (Just (Alias s1)) ExpComparable, symtbl1)
        (Right (Just (Struct ctx), symtbl')) -> structListCheck ctx symtbl'
        Left err -> Left err
      ...
      case getBaseType symtbl (IdOrType s1) of
        (Right (Just (Alias s1), symtbl1)) ->
          case ((Alias s1, Alias s1) `elem` tList) ||
               (binaryAliasHelper symtbl (Alias s1, Alias s1) tList) of
            True -> Right (Just (Alias "bool"), symtbl1)
            False -> Left (InvalidTypeForOpError (Just (Alias s1)) ExpOrdered, symtbl1)
    _ -> Left (DefinitionNotFoundError, symtbl)
binaryCheck tList op (Just t1) symtbl =
  case t1 of
    (Alias s1) ->
      case getBaseType symtbl (IdOrType s1) of
        (Right (Just (Alias s1'), symtbl1)) ->
          case ((Alias s1', Alias s1')) `elem` tList ||
            ...
            False -> Left (InvalidTypeForOpError (Just (Alias s1')) op, symtbl1)
    _ -> Left (DefinitionNotFoundError, symtbl)
