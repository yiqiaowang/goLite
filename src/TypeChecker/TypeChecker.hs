...
class TypeCheckable a b where
  typeCheck
    :: SymbolTable
    -> a
    -> Either (TypeCheckError, SymbolTable) (b, SymbolTable)

  typeCheckList
    :: SymbolTable
    -> [a]
    -> Either (TypeCheckError, SymbolTable) ([b], SymbolTable)
  typeCheckList symtbl [] = Right ([], symtbl)
  typeCheckList symtbl (a:as) = do
    (b, symtbl') <- typeCheck symtbl a
    (bs, symtbl'') <- typeCheckList symtbl' as
    return (b : bs, symtbl'')
  ...
  typeCheckListNewFrame
    ...
    -> [a]
    -> Either (TypeCheckError, SymbolTable) ([b], SymbolTable)
  typeCheckListNewFrame symtbl as = do
    (bs, symtbl') <- typeCheckList (newFrame symtbl) as
    case popFrame symtbl' of
      Right (_, ps) -> Right (bs, ps)
  ...
  --   -> [Type]
  --   -> Either (TypeCheckError, SymbolTable) (b, SymbolTable)
  -- typeCheckElemOf symtbl a types = do
  ...
  --   -> [Type]
  --   -> Either (TypeCheckError, SymbolTable) ([b], SymbolTable)
  -- typeCheckListElemOf symtbl [] _ = Right ([], symtbl)
  -- typeCheckListElemOf symtbl (a:as) types = do
  --   (b, symtbl') <- typeCheckElemOf symtbl a types
  --   (bs, symtbl'') <- typeCheckListElemOf symtbl' as types
  --   return (b : bs, symtbl'')

instance TypeCheckable Program TProgram where
  typeCheck symtbl (Program package alls) = do
    (alls', symtbl') <- typeCheckList symtbl alls
    return (TProgram package alls', symtbl')

instance TypeCheckable All TAll where
  ...
  typeCheck symtbl (Function iden params ret stmts) =
    ...
    case addEntry symtbl (IdOrType iden) functionEntry of
      --TODO validate returns
      Right symtbl' -> case typeCheckList (newFrame symtbl') stmts of
          Right (tStmts, symtbl'') -> case popFrame symtbl'' of
            Right (f, ps) -> Right (TFunction iden, ps)
            Left err -> Left (SymbolTableError err, symtbl'')
          Left err -> Left err
      Left err -> Left (SymbolTableError err, symtbl)

      where
        functionEntry = Entry CategoryVariable $
          Just (Func (functionParamHelper params) ret)
--
